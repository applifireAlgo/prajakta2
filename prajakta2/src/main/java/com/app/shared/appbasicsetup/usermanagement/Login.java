...
@SourceCodeAuthorClass(createdBy = "deepali.arvind@algorhythm.co.in", updatedBy = "", versionNumber = "1", comments = "Login", complexity = Complexity.LOW)
@NamedNativeQuery(name = "complexQuery", query = "SELECT login.appCreatorId FROM ast_Login_T login , ast_CoreContacts_T corecontact,ast_User_T userinfo, ast_UserData_TP userdata WHERE loginId = ? AND login.contactId = corecontact.contactId AND login.userId = userinfo.userId AND userinfo.userId = userdata.userId")
@NamedQueries({ @javax.persistence.NamedQuery(name = "Login.DefaultFinders", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.serverAuthImage LIKE :serverAuthImage"), @javax.persistence.NamedQuery(name = "Login.findByContactId", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.coreContacts.contactId=:contactId"), @javax.persistence.NamedQuery(name = "Login.findByUserId", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.user.userId=:userId"), @javax.persistence.NamedQuery(name = "Login.findById", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.loginPk =:loginPk"), @javax.persistence.NamedQuery(name = "Login.FindUnMappedUser", query = "SELECT u FROM Login u WHERE u.systemInfo.activeStatus=1 AND u.user.userId NOT IN (SELECT ub.userId FROM UserRoleBridge ub)"), @javax.persistence.NamedQuery(name = "Login.FindMappedUser", query = "SELECT u FROM Login u WHERE u.systemInfo.activeStatus=1 AND u.user.userId IN (SELECT ub.userId FROM UserRoleBridge ub)") })
